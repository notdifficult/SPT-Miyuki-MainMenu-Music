...
namespace MiyukiMainMenuMusic.Utils
{
    public static class ConfigManager
    {
        ...
        // В секцию "UI"
        public static ConfigEntry<bool> _showTrackNotification;
        public static ConfigEntry<float> _notificationDuration;
        ...
        public static void Initialize(ConfigFile config)
        {
            ...
            _showTrackNotification = config.Bind(
                "UI", "Show Track Notification", true,
                new ConfigDescription("Показывать всплывающее уведомление о треке (как в NFS)"));

            _notificationDuration = config.Bind(
                "UI", "Notification Duration", 4.0f,
                new ConfigDescription(
                    "Сколько секунд показывать уведомление после смены трека",
